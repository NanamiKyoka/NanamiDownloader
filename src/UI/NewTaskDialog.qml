...
Popup {
    ...
    // 记忆最后的保存位置
    property string lastSavePath: Settings.downloadPath
                ...
                ColumnLayout {
                    ...
                    Item {
                        ...
                        Rectangle {
                            ...
                            Behavior on x {
                                enabled: torrentContent.visible
                                NumberAnimation {
                                    duration: 300
                                    easing.type: Easing.OutCubic
                                }
                            ...
                            ColumnLayout {
                                ...
                                Text {
                                    text: {
                                        if (root.torrentPaths.length === 0)
                                            return qsTr("点击或拖拽 .torrent 文件至此");
                            ...
                            ColumnLayout {
                                ...
                                RowLayout {
                                    ...
                                    RowLayout {
                                        ...
                                        NInput {
                                            id: pathField
                                            text: root.lastSavePath.replace("file:///", "")
                                            Layout.fillWidth: true
                                            onEditingFinished: {
                                                root.lastSavePath = text
                                            }
                                        }
                                        NIconButton {
            ...
            RowLayout {
                ...
                NButton {
                    ...
                    onClicked: {
                        ...
                        var headerObj = {};
                        if (uaField.text !== "")
                            headerObj["User-Agent"] = uaField.text;
                        if (refererField.text !== "")
                            headerObj["Referer"] = refererField.text;
                        if (cookieField.text !== "")
                            headerObj["Cookie"] = cookieField.text;

                        var headerList = [];
                        if (uaField.text !== "")
                            headerList.push("User-Agent: " + uaField.text);
                        if (refererField.text !== "")
                            headerList.push("Referer: " + refererField.text);
                        if (cookieField.text !== "")
                            headerList.push("Cookie: " + cookieField.text);
                        if (headersField.text !== "") {
                            var lines = headersField.text.split("\n");
                            for (var h in lines) {
                                if (lines[h].trim() !== "")
                                    headerList.push(lines[h].trim());
                            }
                        }
                        ...
                        if (hasTask) {
                            // 记忆用户输入的保存路径
                            root.lastSavePath = pathField.text;
                            linkInput.text = "";
                            root.torrentPaths = [];
                            renameField.text = "";
                            root.close();
